...
- **自前サーバーを利用する場合**：「設定 → サーバーアドレス」を開いて `wss://あなたのドメイン` に変更してください（パスは不要、リバースプロキシが処理します）。
...
| 🖥️ **トレイ常駐** | メインウィンドウを閉じるとシステムトレイに格納。左クリックで復帰、右クリックでメニュー。トレイへの最小化に対応。**ステータスカード上のサーバーアドレスはドラッグ選択 / 📋 コピーボタンでコピー可能。** |
